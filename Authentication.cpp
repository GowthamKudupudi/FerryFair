...
int converse (int MsgNum, const struct pam_message** MsgPPtr,
              struct pam_response** RespPPtr, void* DataPtr) {
   return ConversationCntxtPtr ->_converse (
      MsgNum, MsgPPtr, RespPPtr, DataPtr
   );
}
...
Username (Username), Password (Password)
{
   ...
   _State = pam_start (
      "login", Username .c_str(), &Conversation, &_AuthHandlePtr
   );
...
int Authentication_::_converse (
   ...
   if (MsgNum <= 0 || MsgNum > PAM_MAX_NUM_MSG)
      return (PAM_CONV_ERR);
   if ((_RespPtr =
        (pam_response*) calloc (MsgNum, sizeof *_RespPtr)) == NULL)
      return (PAM_BUF_ERR);
